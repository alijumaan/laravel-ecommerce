...
class PageController extends Controller
{
    ...
    {
        $savePage = $this->page->create($request->validated());
        if ($savePage)
            return redirect()->back()->with(['message' => 'Page added successfully', 'alert-type' => 'success',]);

        return redirect()->back()->with(['message' => 'Something was wrong, Please try again later', 'alert-type' => 'danger',]);
    }
    ...
    {
        abort_if(!auth()->user()->can('edit-page'), 403, 'You have not permission to access this page!');
        $this->page::find($page)->first()->update($request->all());
