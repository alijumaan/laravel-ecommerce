                        ...
                        <div class="post-wrapper d-flex">
                                <a href="{{ route('front.products.show', $recent_product->slug) }}">
                                    @if($recent_product->media->count() > 0)
                                        <img class="avatar pb-3" src="{{asset('uploads/products/'. $recent_product->media->first()->file_name) }}"alt="">
                                    @else
                                        <img class="avatar pb-3" src="{{ asset('uploads/default_small.png')}}" alt="">
                                    @endif
                                </a>
                            <div class="">
        ...
        <ul>
            @foreach($recent_comments as $recent_comment)
                <li>
                    <div class="post-wrapper d-flex">
                        <div class="">
                            <img src="{{ get_gravatar($recent_comment->email, 50) }}" alt="{{ $recent_comment->name }}">
                        </div>
                        <div class="ml-3 p-0">
                            @if(isset($recent_comment->product->slug))
                                <a href="{{ route('front.products.show', $recent_comment->product->slug) }}">
                                <h6><span class="text-success">{{ $recent_comment->name }}</span>
                                    <small> review on :
                                    {{ $recent_comment->product->name }}
                                    </small>
                                </h6>
                                <p>{!! \Illuminate\Support\Str::limit($recent_comment->comment, 30, '...') !!}</p>
                                </a>
                            @else
